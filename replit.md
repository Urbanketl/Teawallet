...
- **Session Cookie Domain Fix** - Fixed critical production issue where first-time login + payment caused logout and error despite successful UPI payment. Root cause: Session cookie created on `www.ukteawallet.com` wasn't sent to apex domain `ukteawallet.com` during Razorpay callback. Solution: Set session cookie domain to `.ukteawallet.com` (with leading dot) to work across both www and apex domains. Added `sameSite: 'lax'` to allow cross-site redirects from Razorpay. This resolves the "payment successful but user logged out" issue affecting first-time payment flows.
