...
- **Razorpay Hosted Checkout URL Fixed** - Corrected the checkout endpoint to `https://api.razorpay.com/v1/checkout/embedded` (was incorrectly using `/v1/checkout/pay` which returned 404). This is the proper Razorpay hosted checkout URL that displays QR codes with pre-filled amounts.
