...
- **Payment Link Verification** - Added `/api/wallet/verify-payment-link` endpoint that verifies payment link signature using `razorpay_payment_link_id|razorpay_payment_id` format. Handles payment status check and wallet update after successful verification.
