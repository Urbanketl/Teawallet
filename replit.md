...
- **Unified RFID Reader Support** - Created unified machine controller supporting both ACR122U (USB/PC-SC) and MCRN2 (SPI/PN532) readers with automatic detection. Single codebase works with either reader type without code changes. Implemented proper DESFire APDU response handling with status code validation. Includes comprehensive installation script, systemd service, and documentation for both reader types. Raspberry Pi can now use ACR122U via USB plug-and-play or MCRN2 via SPI connection interchangeably.
- **Session Cookie Domain Fix** - Fixed critical production issue where first-time login + payment caused logout and error despite successful UPI payment. Root cause: Session cookie created on `www.ukteawallet.com` wasn't sent to apex domain `ukteawallet.com` during Razorpay callback. Solution: Updated session configuration to scan full REPLIT_DOMAINS list for custom domain and set cookie domain to `.ukteawallet.com` (with leading dot) to work across both www and apex domains. Added `sameSite: 'lax'` to allow cross-site redirects from Razorpay. This resolves the "payment successful but user logged out" issue affecting first-time payment flows.
- **Callback URL Domain Preference Fix** - Fixed issue where URL changed from www.ukteawallet.com to tea-wallet-prasadthirtha.replit.app during payment flow. Updated callback URL construction in transaction controller to prefer custom domain (ukteawallet.com) over default replit.app domain by scanning full REPLIT_DOMAINS list. Applied domain trimming to handle whitespace in environment variables. Both payment link and payment order flows now maintain custom domain throughout the entire payment process.

## Previous Changes (October 16, 2025)
- **Transaction History Bug Fix** - Fixed critical schema mismatch where `transactions.description` was nullable in Drizzle schema but NOT NULL in database, causing transaction record creation to fail silently. Updated schema to include missing fields: `method`, `rfidCardId`. Wallet balance was updating correctly but transaction history entries were not being created.
- **Razorpay Payment Links Implementation** - Implemented proper redirect payment flow using Razorpay Payment Links API. Backend creates payment link via `/v1/payment_links`, returns `short_url` to frontend, which redirects user to Razorpay hosted checkout. After payment, Razorpay redirects back to callback URL with payment details as query parameters.
- **Payment Link Verification** - Added `/api/wallet/verify-payment-link` endpoint that verifies payment link signature using `razorpay_payment_link_id|razorpay_payment_link_reference_id|razorpay_payment_link_status|razorpay_payment_id` format. Handles payment status check and wallet update after successful verification.
- **Enhanced Error Logging** - Added try-catch block around transaction creation in payment verification to log detailed errors and prevent wallet update failures from blocking payment completion.
- **Backend Callback Handler** - Created `/api/razorpay-callback` GET endpoint that receives payment data from Razorpay and redirects to frontend `/wallet/payment-callback` page with payment parameters as query parameters.
...
- **UPI Sync System**: Automated daily synchronization of UPI payment transactions from Kulhad API at 8 PM IST, with 2-hour overlap window to prevent data gaps.
- **Challenge-Response Authentication**: Full MIFARE DESFire EV1 cryptographic authentication with AES encryption and secure dispensing authorization. AES keys are programmed once at card issuance and stored encrypted in the database using a master key.
- **Timeout Management**: Multi-layered timeout strategy protecting against hung connections and slow operations. Route-specific timeouts (RFID 5s, Analytics 60s, Exports 120s), external API timeouts (WhatsApp, Email, Razorpay 30s), server-level timeouts (request 40s, socket 30s), and database query timeouts (10s execution). Includes comprehensive monitoring service tracking timeout events with admin dashboard.
